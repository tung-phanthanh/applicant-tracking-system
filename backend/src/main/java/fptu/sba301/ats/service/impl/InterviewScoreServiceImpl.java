...
// "Scores can only be submitted for COMPLETED interviews",
// HttpStatus.CONFLICT);
// }

// // 2. Resolve interviewer
// User interviewer =
// userRepository.findByEmailAndDeletedFalse(interviewerEmail)
...
// new InterviewParticipant.InterviewParticipantId(interviewId,
// interviewer.getId()))
// .orElseThrow(() -> new BusinessException("Interviewer is not a participant of
// this interview", HttpStatus.FORBIDDEN));
...
// criterionRepository.findById(item.criterionId())
// .orElseThrow(() -> new BusinessException(
// "Criterion not found with id: " + item.criterionId(), HttpStatus.NOT_FOUND));

// // In our InterviewScore entity, participant is mapped via interview_id and
// user_id fields
// // Wait, does InterviewScore have setting for those fields directly or via
// Participant relation?
// // The InterviewScore entity has: participant, criterion.
// // So we just need to set the participant and criterion.

// Optional<InterviewScore> existingOpt =
// scoreRepository.findByInterviewIdAndParticipantKeyAndCriterionId(
...
// interviewer.getFullName(), participant.getOverallComment(),
// participant.getStrengths(),
...
// buildCriterionNameMap(allScores);
...
// .collect(Collectors.toList());
// Map<java.util.UUID, InterviewParticipant> participantMap =
// participants.stream()
...
// @Override
// @Transactional(readOnly = true)
// public InterviewScoresResponse getMyScores(java.util.UUID interviewId, String
// interviewerEmail) {
// User interviewer =
// userRepository.findByEmailAndDeletedFalse(interviewerEmail)
// .orElseThrow(() -> new BusinessException("Interviewer not found",
...
// private InterviewScoresResponse buildScoresResponse(java.util.UUID
// interviewId,
...
// return new InterviewScoresResponse(interviewId, interviewerId,
// interviewerName,
// overallComment, strengths, weaknesses, recommendation, null, items);
// }

// private Map<java.util.UUID, String>
// buildCriterionNameMap(List<InterviewScore> scores) {
...
// private Map<java.util.UUID, String> buildCriterionMap(List<InterviewScore>
// scores) {
