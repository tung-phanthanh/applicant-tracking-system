...
// @Log4j2
// public class CandidateEvaluationServiceImpl implements
...
// candidateRepository.findById(application.getCandidate().getId())
...
// List<InterviewScore> allScores =
// scoreRepository.findByInterviewIdIn(interviewIds);
...
// for (Map.Entry<java.util.UUID, List<Integer>> entry :
// scoresByCriterion.entrySet()) {
...
// Map<java.util.UUID, List<InterviewScore>> scoresByInterviewer =
// allScores.stream()
// .collect(Collectors.groupingBy(s -> s.getParticipant().getUser().getId()));

// List<InterviewerScoreBreakdown> breakdowns =
// scoresByInterviewer.entrySet().stream().map(entry -> {
...
// return new InterviewerScoreBreakdown(interviewerId, "Interviewer#" +
// interviewerId,
