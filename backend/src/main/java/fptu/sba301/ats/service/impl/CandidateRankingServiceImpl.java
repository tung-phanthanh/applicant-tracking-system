...
// // 2. Load active applications (make sure the repository method exists, using
// findByJobIdAndStatus)
// // Wait, does ApplicationRepository have findByJobIdAndStatus(UUID,
// ApplicationStatus)?
// // The error said `cannot find symbol: method findByJobIdAndStatus` - maybe
// it doesn't exist?
// // Let's use standard JPA repository methods or fetch all by job id and
...
// interviewRepository.findByApplicationIdIn(applicationIds);
...
// Double score = null;
// try {
// EvaluationSummaryResponse eval =
// evaluationService.getEvaluation(app.getId());
// score = eval.aggregateWeightedScore();
// } catch (Exception ignored) {
// // No completed interviews or evaluation error — score stays null
// }

// List<Interview> appInterviews = interviewsByApp.getOrDefault(app.getId(),
...
// app.getStage() != null ? app.getStage().name() :
// ApplicationStage.APPLIED.name(),
// score,
// interviewCount,
// lastInterviewAt);
// }).collect(Collectors.toList());

// // 6. Sort: score DESC (nulls last) → stage ordinal DESC → lastInterviewAt
// DESC
// entries.sort(Comparator
// .comparingDouble((CandidateRankEntry e) -> e.aggregateScore() == null ? -1.0
// : e.aggregateScore())
// .reversed()
// .thenComparing(Comparator.comparingInt((CandidateRankEntry e) ->
// ApplicationStage.valueOf(e.currentStage()).ordinal()).reversed())
...
// }
// }
// ranked.add(new CandidateRankEntry(rank, entry.applicationId(),
// entry.candidateId(),
// entry.candidateFullName(), entry.currentStage(), entry.aggregateScore(),
// entry.interviewCount(), entry.lastInterviewAt()));
// }

// return new CandidateRankingResponse(jobId, job.getTitle(), ranked.size(),
// ranked);
// }
// }
